...
<?if(($arResult['RIGHT_MARGIN'] - $arResult['LEFT_MARGIN']) == 1):?>
	...
	if(($_GET["sort"] == "catalog_PRICE_1") && ($_GET["method"] == "DESC")){
		$sortText = 'цене (макс.>мин.)';
	}
	elseif(($_GET["sort"] == "catalog_PRICE_1") && ($_GET["method"] == "ASC,NULL")){
		$sortText = 'цене (мин.>макс.)';
	}
	elseif(($_GET["sort"] == "name") && ($_GET["method"] == "asc")){
		$sortText = 'наименованию (А-Я)';
	}
	elseif(($_GET["sort"] == "name") && ($_GET["method"] == "desc")){
		$sortText = 'наименованию (Я-А)';
	}
	elseif($_GET["sort"] == "property_PRODUCT_TYPE"){
		$sortText = 'лидеру продаж';
	}
	else{
		$sortText = 'умолчанию';
		$sortFlag = false;
	...
	<div class="cat-sort">
		Сортировать по: 
		<span class="cat-sort-text"><?=$sortText?></span>
		<div class="cat-sort-select"> 
		    <a <?if (($_GET["sort"] == "catalog_PRICE_1") && ($_GET["method"] == "DESC")):?> class="active" <?endif;?>
		        href="<?=$arResult["SECTION_PAGE_URL"]?>?sort=catalog_PRICE_1&method=DESC">цене (макс.>мин.)</a>
		    <a <?if (($_GET["sort"] == "catalog_PRICE_1") && ($_GET["method"] == "ASC,NULL")):?> class="active" <?endif;?>
		        href="<?=$arResult["SECTION_PAGE_URL"]?>?sort=catalog_PRICE_1&method=ASC,NULL">цене (мин.>макс.)</a>
		    <a <?if (($_GET["sort"] == "name") && ($_GET["method"] == "asc")):?> class="active" <?endif;?>
	       		href="<?=$arResult["SECTION_PAGE_URL"]?>?sort=name&method=asc">наименованию (А-Я)</a>
	        <a <?if (($_GET["sort"] == "name") && ($_GET["method"] == "desc")):?> class="active" <?endif;?>
	       		href="<?=$arResult["SECTION_PAGE_URL"]?>?sort=name&method=desc">наименованию (Я-А)</a>
	       	<a <?if ($_GET["sort"] == "property_PRODUCT_TYPE"):?> class="active" <?endif;?>
		       href="<?=$arResult["SECTION_PAGE_URL"]?>?sort=property_PRODUCT_TYPE&method=desc">лидеру продаж</a>
		    <?if($sortFlag):?>
		    	<a href="<?=$arResult["SECTION_PAGE_URL"]?>">умолчанию</a>
			<?endif;?>
		    <span class="cat-sort-select-close"></span>
		</div>
	</div>
	<div class="catalog-list-main">
		<?foreach ($arResult['ITEMS'] as $key => $arItem):
			...
			$minPrice = false;
			?>
			<div class="cat-panel" id="<? echo $strMainID; ?>">
				<div class="cat-panel-izbr sravn-<?=$arItem['ID']?>" onclick="sravn('<?=$arItem['ID']?>');">
					<svg width="22" height="20">
				        <use xlink:href="#izbr"></use>
				    </svg>
				</div>
				<div class="cat-panel-sr sr-<?=$arItem['ID']?>" onclick="sr('<?=$arItem['ID']?>');">
				...
				<a href="<?=$arItem['DETAIL_PAGE_URL']?>" class="cat-panel-link">
					<?if($arItem['PROPERTIES']['CAT_NAL']['VALUE_ENUM_ID'] == 1):?>
						<div class="cat-nal cat-nal-yes">В наличии</div>
					<?else:?>
						<div class="cat-nal cat-nal-no">Под заказ</div>
					<?endif;?>
					<div class="cat-panel-img" style="background: url(<?=$arItem['PREVIEW_PICTURE']['SRC']?>)no-repeat 50%;"></div>
					<div class="cat-panel-name"><?=$arItem['NAME']?></div>
					<div class="cat-panel-info">
						<?if($arItem['PROPERTIES']['CAT_GABARITI']['VALUE']):?>
							<div class="cat-panel-info-item">
								<span>ПВхШхГ, мм</span>
								<span><?=$arItem['PROPERTIES']['CAT_GABARITI']['VALUE']?></span>
							</div>
						<?endif;?>
						<?if($arItem['PROPERTIES']['CAT_OBEM']['VALUE']):?>
							<div class="cat-panel-info-item">
								<span>Объем, л</span>
								<span><?=$arItem['PROPERTIES']['CAT_OBEM']['VALUE']?> л</span>
							</div>
						<?endif;?>
						<?if($arItem['PROPERTIES']['CAT_VES']['VALUE']):?>
							<div class="cat-panel-info-item">
								<span>Вес, кг</span>
								<span><?=$arItem['PROPERTIES']['CAT_VES']['VALUE']?></span>
							</div>
						<?endif;?>
						<?if($arItem['PROPERTIES']['CAT_STVOL']['VALUE']):?>
							<div class="cat-panel-info-item">
								<span>Кол-во стволов</span>
								<span><?=$arItem['PROPERTIES']['CAT_STVOL']['VALUE']?></span>
							</div>
						<?endif;?>
					</div>
					<?
					if(($arItem['PRICES']['BASE']['VALUE'] != '0') && ($arItem['PRICES']['BASE']['VALUE'] != '')):
						$price = "<div class='cat-panel-val'>".number_format($arItem['PRICES']['BASE']['VALUE'], 0, '', ' ')."<span> руб</span></div>";
						$priceVal = number_format($arItem['PRICES']['BASE']['VALUE'], 0, '', ' ');
					else:
						$price = "<div class='cat-panel-val price-zapros'>по запросу</div>";
					endif;
					?>
					<div class="cat-panel-price">
						<div class="cat-panel-text">Цена:</div>
						<?=$price?>
					</div>
				</a>
				<?if($arItem['PROPERTIES']['CAT_NAL']['VALUE_ENUM_ID'] == 1):?>
					<div class="recommend-panel-btn" onclick="add2(1,'<?=$arItem['NAME']?>','','<?=$arItem['ID']?>', this,'<?=$arItem['PRICES']['BASE']['VALUE']?>');">В корзину</div>
				<?else:?>
					<div class="recommend-panel-btn btn-zakaz" onclick="fosTovar('Под заказ','Под заказ','CLICK1','<?=$arResult['NAME']?>','<?=$priceVal?> руб.');">Под заказ</div>
				<?endif;?>
			</div>
		<?endforeach;?>
	</div>
