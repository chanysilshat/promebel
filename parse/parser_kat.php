...
$counter=file_get_contents('counter_kat.txt');
...
echo '='.$counter;	
	...
	$count1=0;
	$index_assoc=$counter;
	echo '<p>index_assoc='.$index_assoc.'</p>';
	foreach($tmp as $value){	
	$count1++;
	$url=$value[img];
	$url_mass=explode("/", $url);
	$file_name=end($url_mass);
	$text.=$id_sect.';'.$index_assoc.';'.$value[name].';'.$value[img].';'.$value[href]."\n";

		$ch = curl_init('https://baxi.ru/'.$value[img]);//откуда качаем
		$fp = fopen('storage/img-kat/'.$file_name, 'wb');
		curl_setopt($ch, CURLOPT_FILE, $fp);
		curl_setopt($ch, CURLOPT_HEADER, 0);
		curl_exec($ch);
		curl_close($ch);
		fclose($fp);		
		$images_url='storage/img-kat/'.$file_name;	
	
	
	
	
	$array_xml[$index_assoc]['id']=$index_assoc;
	$array_xml[$index_assoc]['name']=$value[name];
	$array_xml[$index_assoc]['image']=$images_url;
...
$xml_user_info = new SimpleXMLElement("<?xml version=\"1.0\" encoding=\"UTF-8\"?><category></category>");

//function call to convert array to xml
array_to_xml($array_xml,$xml_user_info);

//saving generated xml file
$xml_file = $xml_user_info->asXML('category.xml');
...
file_put_contents('counter_kat.txt',$index_assoc);	
...
echo '<p>'.$x.'</p>';	
$arSelect_1 = Array('ID', 'NAME','UF_OLD_URL','PICTURE','IBLOCK_SECTION_ID');
$arFilter_1 = Array('IBLOCK_ID'=>45, 'GLOBAL_ACTIVE'=>'Y', 'GLOBAL_ACTIVE'=>'Y','DEPTH_LEVEL'=>$x);
...
$ss_sect_rod='';
$ss_sect_rod=$mass_row1['IBLOCK_SECTION_ID'];
$images_url=CFile::GetPath($mass_row1["PICTURE"]);
$url_mass=explode("/", $images_url);
$file_name=end($url_mass);
		$ch = curl_init('https://fors-tsk.ru'.$images_url);//откуда качаем
		$fp = fopen('storage/img-kat/'.$file_name, 'wb');
		curl_setopt($ch, CURLOPT_FILE, $fp);
		curl_setopt($ch, CURLOPT_HEADER, 0);
		curl_exec($ch);
...
if ($ss_sect_rod!=''){
	$name_sec='';
	$arSelect_2 = Array('ID', 'NAME','UF_OLD_URL','PICTURE','IBLOCK_SECTION_ID');
	$arFilter_2 = Array('IBLOCK_ID'=>45, 'GLOBAL_ACTIVE'=>'Y', 'GLOBAL_ACTIVE'=>'Y','ID'=>$ss_sect_rod);
	$row2 = CIBlockSection::GetList(Array("ID"=>"DESC"), $arFilter_2, false, $arSelect_2, Array("nPageSize"=>1));	
	while($mass_row2 = $row2->GetNext())
	{
		$name_sec=$mass_row2['NAME'];
	...
	$array_xml[$index_assoc]['id']=$mass_row1['ID'];
	$array_xml[$index_assoc]['name']=$mass_row1['NAME'];
	$array_xml[$index_assoc]['image']=$images_url;
	if ($ss_sect_rod!=''){	
	$array_xml[$index_assoc]['categories']['id']=$ss_sect_rod;
	$array_xml[$index_assoc]['categories']['name']=$name_sec;
	}
...
//saving generated xml file
$xml_file = $xml_user_info->asXML('category_new.xml');
