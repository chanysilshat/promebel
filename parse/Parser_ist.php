...
function pars($id_sect,$ss_sect){
    $url_parce='http://www.veterinarka.ru/vetconf/1.html';
    $html = Parser::getPage([
        "url" => $url_parce
    ]);
...
echo '<p>22</p>';
            $product = pq($product);
                    $tmp[] = [
                        //"href" => trim($product->find("a")->attr("href")),
                        //"img" => trim($product->find("img")->attr("src")),
                        "name" => trim($product->find(".item-title")->text()),
...
foreach($tmp as $value){
   //  $count1++;
   // $aa.=$count1.';'.$value[name].';'.$value[text]."\n";
echo "<pre>";
 print_r($value);
echo "</pre>";
