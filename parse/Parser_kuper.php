...
function pars($id_sect,$ss_sect){
    ...
    $url_parce='http://www.rp.ru'.$ss_sect;
	echo '<p>'.$url_parce.'</p>';
    $html = Parser::getPage([
        "url" => $url_parce
    ]);
    ...
    if(!empty($html["data"])){
		...
		$pq=phpQuery::newDocumentHTML($content,'UTF8');
...
file_put_contents($count, $count1); 
	...
	$file_pars = 'text_c.txt';
	file_put_contents($file_pars, $i); 
	}
